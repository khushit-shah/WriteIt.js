...
class WriteIt {
  ...
  constructor() {
    ...
    this.WRITEIT_AUTO_START = "writeit-auto-start";
  ...
  pauseAnimation(node) {
    ...
    this.writeitNodes.forEach(_node => {
      if (_node.node == node) {
        // Stop animation of animating node.
        _node.pauseAnimation();
        return false;
      }
    });
  }
  ...
  init() {
    ...
    // Make the nodes declared with WRITEIT_HIDDEN hidden.
    document.querySelectorAll("*[" + this.WRITEIT_HIDDEN + "]").forEach(element => {
...
class WriteItNode {
  ...
  animate() {
    ...
    // Remove the writeitChar.
    str = str.substr(0, str.length - this.writeitChar.length);
    ...
    if (this.reverse) {
      ...
      if (this.node.hasAttribute(WriteItJS.WRITEIT_REPLACE_NEXT)) {
        ...
        if (
          ...
          this.textsIndex = 0;
          this.text = this.texts[this.textsIndex];
          console.log(this.textsIndex, this.texts);
          this.trigerNextAnimation(false, 0, true);
        } else {
  ...
  animationEnd() {
    if (this.node.hasAttribute(WriteItJS.WRITEIT_NEXT)) {
      ...
      let elem = this.node.getAttribute(WriteItJS.WRITEIT_NEXT).split(",");
      elem.forEach(e => {
        let node = document.querySelector(e);
        new WriteItNode(node, true).startAnimation();
      });
    } else {
