class Cursor {
  ...
  startAnimation() {
    for (var t in this.nodes)
      this.nodes.hasOwnProperty(t) &&
        (this.nodes[t].hasAttribute("cursor-before")
          ? ((this.nodes[t].style.display = "none"),
            (this.nodesAnimation[t] = this.nodes[t].id))
          : (this.nodesAnimation[t] = new Node(this.nodes[t], this.texts[t])));
  }
  reInitializeALL() {
    for (var t in this.nodes)
      this.nodes.hasOwnProperty(t)
        ? this.nodes[t].hasAttribute("cursor-before") ||
          (this.nodesAnimation[t].destruct(),
          (this.nodesAnimation[t] = new Node(this.nodes[t], this.texts[t])))
        : ((this.nodes[t].style.display = "none"),
          (this.nodesAnimation[t] = this.nodes[t].id));
  }
  reInitialize(t) {
    try {
      ...
      if (e.hasAttribute("cursor-animate")) {
        ...
        0 <= s
          ? (console.log(s),
            console.log(this.nodesAnimation[s]),
            console.log(this.nodesAnimation),
            e.hasAttribute("cursor-before")
              ? (console.log("have cursor-before"),
                (this.nodes[s].innerHTML = ""),
                (this.nodes[s].style.display = "none"))
              : (this.nodesAnimation[s].destruct(),
                (this.nodesAnimation[s] = new Node(e, this.texts[s]))))
  ...
  addCBefore(t) {
    for (var e = t.id, s = 0; s < this.nodesAnimation.length; s++) {
      ...
      this.nodes[s] == t &&
        (i != e && this.nodesAnimation[s].destruct(),
        (t.innerHTML = ""),
        (this.nodesAnimation[s] = new Node(t, this.texts[s])));
    }
  }
}
var cursor,
  Node = function(t, e) {
    if (void 0 == t || null == t)
      throw Error("Node Must be a valid HTML Element");
    (this.node = t),
      (this.text = e),
      (this.interval = null),
      (this.texts = []),
      ...
      (this.nodeInnerHTML = ""),
      this.setSpeed(),
      this.node.hasAttribute("cursor-replace-next") &&
...
(Node.prototype.destruct = function() {
    ...
    if (this.node) {
      if (((this.nodeInnerHTML = this.node.innerHTML = ""), this.reverse)) {
        ...
        for (var t = this.index; 0 <= t; t--) {
          if (">" == this.text[t])
            for (; "<" != this.text[t]; )
              (this.nodeInnerHTML += this.text[t]), t--;
          this.nodeInnerHTML += this.text[t];
        }
        (this.nodeInnerHTML = this.nodeInnerHTML
          .split("")
          .reverse()
          .join("")),
          (this.nodeInnerHTML += "|");
        ...
        if (((this.nodeInnerHTML += "|"), "<" == this.text[this.index])) {
          for (; ">" != this.text[this.index]; ) this.index++;
          this.index++;
        }
      }
    ...
    if (this.reverse)
      if ((this.index--, 0 > this.index))
        if (this.node.hasAttribute("cursor-replace-next")) {
          var t;
          for (
            console.log(this.text, this.texts), t = 0;
            t < this.texts.length;
            t++
          ) {
            if (this.texts[t] == this.text)
              return (
                (this.index = 0),
                (this.reverse = !1),
                (this.text =
                  t + 1 == this.texts.length
                    ? this.texts[0]
                    : this.texts[t + 1]),
                void this.setTimeout(this.speed)
              );
          ...
          (this.index = 0), (this.reverse = !1), this.setTimeout(this.speed);
      ...
      if (this.node.hasAttribute("cursor-after"))
        try {
          ((t = document.querySelector(
            this.node.getAttribute("cursor-after")
          )).style.display = ""),
            cursor.addCBefore(t);
        } catch (t) {
          console.log(t);
      ...
      if (this.node.hasAttribute("cursor-replace-next"))
        if (this.text == this.texts[this.texts.length - 1]) {
          ...
          if (this.node.hasAttribute("cursor-loop")) {
            ...
            this.interval = setInterval(function() {
              e.node.innerHTML =
                e.node.innerHTML == e.text || e.node.innerHTML == e.text + " "
                  ? e.text + "|"
                  : e.text + " ";
            }, 500);
          }
        ...
        (this.index = 0), this.setTimeout(this.speed);
      else if (this.node.hasAttribute("cursor-loop-reverse"))
        (this.index = this.text.length - 1),
          (this.reverse = !0),
          this.setTimeout(this.speed);
      else if (this.node.hasAttribute("cursor-after"))
        this.node.innerHTML = this.text;
      else {
        var s = this;
        this.interval = setInterval(function() {
          s.node.innerHTML =
            s.node.innerHTML == s.text || s.node.innerHTML == s.text + " "
              ? s.text + "|"
              : s.text + " ";
        }, 500);
      }
  ...
  (Node.prototype.setSpeed = function() {
    if (this.node.hasAttribute("cursor-speed-fixed"))
      this.speed = 1e3 / this.node.getAttribute("cursor-speed-fixed");
